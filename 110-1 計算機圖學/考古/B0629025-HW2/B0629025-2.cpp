...
void displayFcn (void) {
	...
	for (int j=0;j<360;++j)
            {
            	verts[j] = {100+100*cos(2*pi/360*j),100+100*sin(2*pi/360*j) };
                glVertex2f(100+100*cos(2*pi/360*j),100+100*sin(2*pi/360*j)); //CIRCLE
            }
	...
	scale2D (2.0, 2.0, fixedPt);   //  First transformation: Scale.
	rotate2D (pivPt, -(2*theta));     //  Second transformation: Rotate
	translate2D (150, 50);        //  Final transformation: Translate.
	/*  Apply composite matrix to triangle vertices.  */
	transformVerts2D (nVerts, verts);
	glColor3f (0.0, 1.0, 0.0);  // Set color for transformed triangle.
